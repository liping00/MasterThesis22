...
for k = 1:N
	...
	if(bprbs==0)
        if k<1000
            u1=0;
            u2=0;
        elseif(k==1000)       % Bump less transfer 
            e1=r-y1;
            e2=r-y2;
   ...
   if k>1800
        u1=u1-30*10^7;  % simulate the disturbance 
        u2=u2+50*10^7;
   end
    Y1(k,1)=y1; U1(k,1)=u1; R(k)=r;  % store data 
    Y2(k,1)=y2; U2(k,1)=u2; 
    tau1 = [u1,0,0]';
    tau2 = [0,u2,0]';
    [Time,sol] = ode45(@supply1,[tk tk+dt],x1',[],tau1); % discretise the model
    x1 = sol(end,:)'+v(k)';  % update model with last element
    [Time,sol] = ode45(@supplytest1,[tk tk+dt],x2',[],tau2);
    x2 = sol(end,:)'+v(k)';    
...
figure(2)
subplot(211),plot(1:N,Y1,1:N,Y2,'r-',1:N,R,'k--'),title('$y$: position [m]','Interpreter','latex'),ylabel('$y$','Interpreter','latex'),grid,t=legend('x direction','y direction');set(t,'Interpreter','latex'),xlim([1,N])
